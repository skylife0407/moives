	...
	<head>
		...
		<script>
			function doActivity(target , boardNo){
				...
				xmlhttp.open("get" , "./doActivityFreeBoard.do?board_no=" + boardNo + "&activity=" + target); 
				xmlhttp.send();
			}
					...
					<div class="col-10 userInfofont">
						...
						<c:choose>
							<c:when test="${sessionUser.member_auth == 1 }">
								<div class="row mt-3">
									<div class="col text-center fs-4">자유게시판 관리</div>
								</div>
								...
								<div class="row mt-3">
									<div class="col">
										<table class="table table-striped">
											...
											<c:forEach items="${list }" var="freeboarddate">
												<tr>
													<td><a href="/movie/board/freeboard/readContentPage.do?freeboard_no=${freeboarddate.adminFreeBoardListVo.freeboard_no }">${freeboarddate.adminFreeBoardListVo.freeboard_no }</a></td>
													<td>${freeboarddate.adminFreeBoardListVo.freeboard_title }</td>
													<td>${freeboarddate.adminMemberListVo.member_nick }</td>
													<td><fmt:formatDate pattern="yyyy-MM-dd hh:mm:ss" value="${freeboarddate.adminFreeBoardListVo.freeboard_writedate }" /></td>
													<td>
														<p id="open" onclick="doActivity(0,${freeboarddate.adminFreeBoardListVo.freeboard_no })" class="btn<c:if test="${freeboarddate.adminFreeBoardListVo.freeboard_activity == 0 }" > btn-primary</c:if>">공개</p>
														<p id="close" onclick="doActivity(1,${freeboarddate.adminFreeBoardListVo.freeboard_no })" class="btn<c:if test="${freeboarddate.adminFreeBoardListVo.freeboard_activity == 1 }" > btn-primary</c:if>">비공개</p>
													</td>
												</tr>
											</c:forEach>
										</table>
